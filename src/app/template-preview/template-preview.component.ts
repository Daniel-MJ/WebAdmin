...
export class TemplatePreviewComponent implements OnInit {
  ...
  sendTemplate() {
    ...
    this.apiService.sendToTwitter(this.template).subscribe(
      response => {
        console.log('Plantilla enviada a Twitter correctamente:', response);
      },
      error => {
        console.error('Error al enviar la plantilla a Twitter:', error);
      }
    );
